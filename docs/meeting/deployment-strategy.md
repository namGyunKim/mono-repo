...
코드 한 줄이 사용자에게 도달하기까지의 과정이다.

```
1. 개발        2. 코드 리뷰       3. 자동 검사(CI)     4. 머지          5. 배포
─────────    ──────────      ─────────────     ────────      ──────────
feat 브랜치 → PR 생성        → 빌드·테스트·E2E  → develop 반영 → 배포 브랜치 push
에서 코드 작성   (변경 내용 확인)   (3개 모두 통과 필수)  (squash merge)   → 서버에 자동 반영
```

| 단계            | 누가 하나     | 무슨 일이 일어나나                       |
|---------------|-----------|----------------------------------|
| **1. 개발**    | 개발자       | feat 브랜치에서 기능 개발                 |
| **2. PR 생성** | 개발자       | develop 브랜치로 머지 요청 (변경 내용 리뷰)    |
| **3. CI 검사** | GitHub 자동 | 백엔드·프론트엔드·E2E 3단계 자동 검사         |
| **4. 머지**    | 개발자       | CI 통과 후 develop에 반영 (커밋 1개로 합침) |
| **5. 배포**    | 개발자       | 배포 브랜치에 push → 서버에 자동 배포         |

> 3단계에서 하나라도 실패하면 머지 버튼이 비활성화되어 **문제 있는 코드가 합쳐지는 것을 원천 차단**한다.
> 실서버 긴급 수정이 필요할 때는 별도의 **hotfix 흐름**을 사용한다 (아래 "긴급 수정" 참조).
...
배포 브랜치에 push하면, **올바른 소스에서 병합되었는지 자동으로 검증**한 후 배포를 진행한다.
잘못된 브랜치에서 병합하면 배포가 차단된다.

| 배포 브랜치    | 허용된 병합 소스 | 차단 예시                          |
|------------|-----------|--------------------------------|
| `stage/*`  | `develop` | feat 브랜치 직접 병합, main 병합        |
| `deploy/*` | `main`    | develop 직접 병합, feat 브랜치 직접 병합  |

> GitHub Actions의 수동 실행(workflow_dispatch)은 검증을 건너뛴다. 긴급 재배포 시 활용할 수 있다.

### 긴급 수정 (hotfix)

실서버에서 버그가 발견되어 즉시 수정해야 할 때 사용하는 흐름이다.

```
1. hotfix 브랜치 생성 (main 기반)
2. 수정 후 main에 PR → 머지
3. main → deploy/* 병합 → 실서버 배포
4. main → develop 역병합 PR → 수정 내용 동기화
```

| 단계         | 누가 하나 | 무슨 일이 일어나나                      |
|------------|-------|----------------------------------|
| **1. 수정**  | 개발자   | main에서 hotfix 브랜치 생성, 버그 수정     |
| **2. PR**  | 개발자   | main으로 PR 생성 & 머지               |
| **3. 배포**  | 개발자   | deploy 브랜치에 main 병합 & push → 자동 배포 |
| **4. 동기화** | 개발자   | main → develop 역병합 PR (수정 내용 반영) |

> 역병합을 빠뜨리면 다음 정기 릴리스에서 수정이 사라질 수 있으므로 반드시 수행한다.
...
서버에 새 버전을 올릴 때, **기존 버전(Blue)을 끄지 않고 새 버전(Green)을 옆에 띄운 뒤 트래픽을 전환**하는 방식이다.
새 버전에 문제가 있으면 다시 기존 버전으로 돌리면 되므로, **사용자가 서비스 중단을 체감하지 않는다.**

쉽게 비유하면: 고속도로에서 차선 하나를 보수할 때, 옆 차선으로 우회시킨 뒤 작업하고, 끝나면 다시 돌리는 것과 같다.
