...
class ControllerLogMessageFactoryTest {
    ...
    void buildRequestLog_contains_REQ_prefix() {
        final String result = ControllerLogMessageFactory.buildRequestLog("abc12345", "127.0.0.1", "user1", "GET", "/api/members", "page=1");
        assertThat(result).contains("[REQ]");
    }

    @Test
    void buildRequestLog_contains_all_fields() {
        final String result = ControllerLogMessageFactory.buildRequestLog("abc12345", "127.0.0.1", "user1", "POST", "/api/members", "{}");
        assertThat(result).contains("abc12345", "127.0.0.1", "user1", "POST", "/api/members");
    }

    @Test
    void buildResponseLog_contains_RES_prefix() {
        final String result = ControllerLogMessageFactory.buildResponseLog("abc12345", 150, "200", "1024B");
    ...
    void buildResponseLog_contains_status_and_size() {
        final String result = ControllerLogMessageFactory.buildResponseLog("abc12345", 50, "201", "512B");
